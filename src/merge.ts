...
export class ThreeWayMerger {
   ...
   */
  static merge(base: string, local: string, remote: string): { merged: string; conflicts: boolean } {
...
export class MarkdownMerger {
   ...
   */
  static merge(local: string, remote: string, base: string): { merged: string; conflicts: boolean } {
    ...
    // Merge content
    const { merged: mergedContent, conflicts } = ThreeWayMerger.merge(
      baseContent,
      localContent,
      remoteContent
    );
  ...
  private static extractFrontmatter(content: string): { frontmatter: Record<string, any>; content: string } {
