...
export class SimpleCronParser {
    ...
    static parseField(field, min, max) {
        ...
        if (field.includes('/')) {
            ...
            if (base === '*') {
                return Array.from({ length: Math.ceil((max - min + 1) / stepNum) }, (_, i) => Math.min(min + i * stepNum, max));
            }
        }
        // Single number
     ...
     */
    static matches(cronExpr, now = new Date()) {
...
export class SyncScheduler {
     ...
     */
    addIntervalSchedule(name, intervalMs) {
     ...
     */
    addCronSchedule(name, cronExpression) {
        // Validate cron
    ...
    scheduleTask(name, schedule) {
        if (schedule.type === 'interval' && schedule.intervalMs) {
            ...
            const timer = setInterval(async () => {
                if (!schedule.enabled || !this.onSyncCallback)
                    return;
                try {
                    schedule.lastRunTime = Date.now();
     ...
     */
    calculateNextCronTime(cronExpr) {
